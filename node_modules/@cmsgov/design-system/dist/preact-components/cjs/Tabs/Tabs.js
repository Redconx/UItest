...
const getDefaultSelectedId = props => {
  ...
  _compat.Children.forEach(props.children, function (child) {
...
const Tabs = props => {
  ...
  const [internalSelectedId, setSelectedId] = (0, _compat.useState)(initialSelectedId);
  ...
  const tabsRef = (0, _compat.useRef)({});
  ...
  const panelChildren = () => {
    return _compat.Children.toArray(props.children).filter(isTabPanel);
  };
  ...
  const handleTabKeyDown = (evt, panelId) => {
    const tabs = panelChildren().filter(elem => _compat.default.isValidElement(elem));
  ...
  const renderChildren = () => {
    return _compat.Children.map(props.children, child => {
      if (isTabPanel(child) && (0, _compat.isValidElement)(child)) {
