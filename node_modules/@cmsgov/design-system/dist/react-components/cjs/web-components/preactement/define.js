...
var _kebab = require("case-it/kebab");
 ...
 */
function define(tagName, componentFunction) {
  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  const elementTag = (0, _shared.getElementTag)(tagName);
...
function createServerSideRenderFunction(elementTag, componentFunction) {
  ...
  if ((0, _shared.isPromise)(component)) {
    throw new Error(`${_shared.ErrorTypes.Promise} : <${elementTag}>`);
  }
  const {
    wrapComponent
  } = options;
  ...
  return props => (0, _preact.h)(elementTag, attributes, [(0, _preact.h)('script', {
    type: 'application/json',
    dangerouslySetInnerHTML: {
      __html: JSON.stringify(props)
    }
  }), (0, _preact.h)(component, props)]);
}
...
function createCustomElement(componentFunction) {
  ...
  class TheCustomElement extends HTMLElement {
    ...
    renderPreactComponent(addedNodes) {
...
function proxyEvents(props, events, CustomElement) {
  ...
  (events || []).forEach(nameOrArray => {
    ...
    const customName = (0, _kebab.kebabCaseIt)(name.replace('on', 'ds'));
...
async function onConnected() {
  const attributes = _shared.getElementAttributes.call(this);
  const propsAttribute = this.getAttribute('props');
  const json = this.querySelector('[type="application/json"]');
  const data = _shared.parseJson.call(this, propsAttribute || json?.innerHTML || '{}');
  ...
  if ((0, _shared.isPromise)(component)) {
    component = await (0, _shared.getAsyncComponent)(component, this.tagName);
  }
  if (!component) {
    console.error(_shared.ErrorTypes.Missing, `: <${this.tagName.toLowerCase()}>`);
    return;
  }
  const {
    wrapComponent
...
function onAttributeChange(name, _original, updated) {
  ...
  if (name === 'props') {
    props = {
      ...props,
      ..._shared.parseJson.call(this, updated)
    };
  } else {
    props = {
      ...props,
      [(0, _shared.getPropKey)(name)]: updated
    };
  }
 ...
 */
function renderPreactComponent(addedNodes) {
  if (!this.__component) {
    console.error(_shared.ErrorTypes.Missing, `: <${this.tagName.toLowerCase()}>`);
    return;
  }
  ...
  let template = [...this.childNodes].find(isTemplate);
  ...
  const {
    vnode,
    slots
  } = (0, _parse.templateToPreactVNode)(template);
  ...
  const propsSignal = (0, _signals.signal)(this.__properties);
  this.__propsSignal = propsSignal;
  const StateWrapper = () => (0, _preact.h)(this.__component, {
    ...propsSignal.value,
    ...slots,
    children
  });
  ...
  (0, _preact.render)((0, _preact.h)(StateWrapper, {}), this);
